...
- [x] 全部 Lecture 每门 ≥2 名 staff 教师（见 §4.3 矩阵；现行 32 Lecture）
...
- [x] LMS 全量选课核对：AIEDU Lecture active 选课 2581 条（重构前 16 Lecture 快照，现行为 32 Lecture）（P1 164 / P2 160 / P3 163 / P4 169 / P5 169 / P6 166 / A1~A4、B1~B6 各 159）
...
| 教师账户 | 21 | teacher_zhang 等 8 + teacher_ai_01/02 + lecture_p1~p6 | instructor/staff 角色（32 Lecture 角色分配见 OPERATION-GUIDE §18） | all-teachers + course-*-teachers（teacher_ai_01/02 为 Hub admin） |
...
- py_a_001 为超级样本账号：全部 Lecture 选课，用于教师演示；py_a_002~050 仅 P1。
...
| 账户 | 邮箱 | 角色 |
|---|---|---|
| teacher_zhang | teacher-zhang@edu.local | 全部 32 个 Lecture staff（教师1，矩阵 §4.3） |
...
| Lecture-A1~A12 | （course instructor 账户，历史命名） | A 系列各 Lecture（现行 instructor=teacher_ai_01/02 + teacher_zhang） |
| Lecture-B1~B12 | （course instructor 账户，历史命名） | B 系列各 Lecture（现行 instructor=teacher_java_01/02 + teacher_zhang） |
...
| T2b | 全部 Lecture 每门 ≥2 教师（staff/instructor 角色逐课核验；2026-09-17 起为 32 个 Lecture） | ✅ PASS |
