#!/usr/bin/env python3
"""
Phase 2: Upgrade 5 advanced-chat apps with Chatflow workflow graphs (start->llm->answer)
+ industrial-grade system prompts.
Phase 3: Upgrade 3 workflow apps with workflow graphs (start->llm->end).
"""
import requests, base64, json, sys, time, uuid
...
def make_advanced_chat_graph(system_prompt, model, temp=0.6):
    """Build a Chatflow graph: start -> llm -> answer."""
...
def make_workflow_graph(system_prompt, model, input_vars, temp=0.6):
    """Build a workflow graph: start(with inputs) -> llm -> end(with outputs).
    input_vars: list of {variable, label, type, required, max_length}
    """
    start_id = str(int(time.time()*1000))
    end_id = str(int(time.time()*1000)+1)
    user_query_var = "{{#sys.query#}}" if not input_vars else "{{#%s.%s#}}" % (start_id, input_vars[0]["variable"])
    ...
    # Build the user message content referencing the main input variable
    if input_vars:
        msg_text = "{{#%s.%s#}}" % (start_id, input_vars[0]["variable"])
    else:
        msg_text = "{{#sys.query#}}"
    ...
    return {
        "nodes": [
            ...
            },
            {
                ...
                "data": {
                    "outputs": [{"value_selector": ["llm","text"], "value_type": "string", "variable": "result"}],
...
def update_workflow_draft(aid, graph, features, env_vars=None, conv_vars=None):
    payload = {"graph": graph, "features": features,
               "environment_variables": env_vars or [], "conversation_variables": conv_vars or []}
    r = s.post(f"{BASE}/console/api/apps/{aid}/workflows/draft", headers=H, json=payload)
    return r.status_code, r.text[:400]

# ============================================================
# PROMPTS for the 5 ADVANCED-CHAT apps
# ============================================================
PROMPTS_AC = {}

PROMPTS_AC["工业互联网智能体"] = """你是一名工业互联网领域的高级智能体(Agent),具备自主推理、工具调用与多步任务编排能力,集成工业知识图谱、协议解析、设备建模与故障诊断等工具,服务于工业互联网平台的工程师与运维人员,覆盖设备接入、数据治理、应用开发、安全运营全场景。

【智能体核心能力】
1. 设备接入与协议转换:能解析Modbus寄存器表、OPC UA节点空间、MQTT Topic结构,生成设备模型与点位映射,支持协议网关配置(如Modbus转OPC UA)。
2. 工业知识检索:掌握IEC 61131、IEC 62443、ISA-95、ISO 23247等标准,能根据问题检索并引用条款,关联行业实践。
3. 故障诊断推理:基于故障现象与设备拓扑,运用故障树(FTA)与根因分析(RCA),给出排查路径与验证方法。
4. 数据分析编排:能调用时序数据查询、统计计算、ML预测等工具,组合成多步分析流程并解读结果。
5. 工程化输出:生成可执行配置(网关脚本、PLC变量表、看板JSON)、技术方案文档、测试用例。

【多步骤推理回答规范】
1. 任务理解:复述并结构化用户意图,识别涉及的领域(接入/数据/分析/安全/应用)与所需工具。
2. 信息收集:主动说明需要哪些输入(设备型号、点位表、日志、报错码),缺失时给出合理假设并标注。
3. 推理链路:逐步推理——现象→可能原因(按概率排序)→验证方法→结论,每步说明依据。
4. 工具调用:当问题需要协议解析、数据查询、标准检索时,说明将调用何种工具及预期输出。
5. 方案输出:给出结构化方案(步骤+配置+验证+回滚),标注关键参数与风险点。
6. 边界与升级:明确智能体能力边界,对需人工介入(如现场接线、硬件更换、安全停车)的事项给出交接清单。

【交互准则】
- 复杂任务先给出执行计划再逐步执行,允许用户中途修正。
- 所有涉及安全的操作(急停、联锁、防爆)给出强制人工确认提示。
- 输出配置/脚本前标注适用版本与前置条件。"""
...
PROMPTS_AC["工业生产线优化顾问"] = """你是一名工业生产线优化顾问智能体,精通精益生产(Lean)、TOC(约束理论)、IE(工业工程)手法、Six Sigma与数字化制造,能在产能、节拍、良率、成本、交付多目标下为离散与流程生产线提供系统优化方案,具备从价值流分析到产线平衡到自动化升级的全栈能力。

【核心方法库】
1. 诊断方法:价值流图(VSM)、节拍时间(Takt)与周期时间(CT)对比、OEE拆解(可用率/表现/质量)、瓶颈识别(CT最长工位)、产线平衡率与平衡损失率、排队论(WIP/Little定律)。
2. 优化工具:IE七大手法(流程/搬运/操作/检验/停滞/搬运/材料)、防错(Poka-Yoke)、快速换模(SMED)、5S与目视化、单元化生产(Cell)、拉动系统(看板Kanban)、JIT准时化。
3. 质量优化:六西格玛DMAIC、SPC过程能力(Cp/Cpk/Ppk)、DOE试验设计、FMEA、8D根因分析。
4. 自动化与数字化:机器人单元评估、AGV/AMR物流方案、MES数据采集、数字孪生仿真(Plant Simulation/FlexSim),以及ROI测算。
5. 排产与计划:APS高级排产、TOC DBR(鼓-缓冲-绳)、混合流水线平衡。

【多步骤顾问回答规范】
1. 现状诊断:基于用户描述的产线数据(工时/节拍/良率/OEE/在制品/订单),识别瓶颈与浪费(过量生产/等待/搬运/库存/过度加工/动作/缺陷),量化损失。
2. 目标设定:结合Takt与订单需求,设定优化目标(如OEE从65%到80%、CT降低15%、良率+3%),标注优先级与约束(投资/停产窗口/人员)。
3. 方案设计:给出2-3套方案(快速改善/中期IE优化/长期自动化),每套含措施清单、预期收益、投资、风险、实施周期。
4. 实施路径:按SMED/5S/平衡/自动化分阶段,给出里程碑、试点建议与推广策略,标注变革管理要点(培训/激励/标准化)。
5. 量化收益:测算产能提升、人力节省、在制品降低、质量成本降低,给出ROI与回收期,标注数据假设。
6. 持续改善:建立KPI看板、日常管理(DM)与改善周机制,形成持续改善文化。

【沟通准则】
- 顾问式、数据驱动,避免无依据的乐观承诺,所有收益标注假设与不确定性。
- 优先"低投资高回报"的IE改善,再评估自动化,反对盲目堆砌设备。
- 结合数字化但不脱离管理基础(自动化是放大器,精益是地基)。"""

PROMPTS_AC["工业安全合规检查员_b"] = PROMPTS_AC["工业安全合规检查员"]  # alias

# Advanced-chat apps: id -> (name, model)
AC_APPS = [
    ("4e63c4d7-baf7-41b1-b295-c4565f72877e", "工业互联网智能体", M_REASON),
    ("15b2a9b9-2823-4d55-8c10-30ff760e7382", "工业设备预测性维护智能体", M_REASON),
    ("2dd72f64-61b6-46e8-8841-8300df3d921a", "工业安全合规检查员", M_REASON),
    ("6fe85559-7f51-4849-a3d3-3c596853864a", "工业生产线优化顾问", M_REASON),
]

print("=" * 60)
print("PHASE 2: Upgrading 4 ADVANCED-CHAT apps (workflow graph + prompt)")
print("=" * 60)
ok = 0; fail = 0
for aid, name, model in AC_APPS:
    ...
    code, resp = update_workflow_draft(aid, graph, features)
    plen = len(prompt)
    if code == 200:
        print("  [OK] %-26s model=%s prompt_len=%d" % (name, model, plen)); ok += 1
    else:
        print("  [FAIL] %-26s code=%d resp=%s" % (name, code, resp)); fail += 1
    time.sleep(0.5)
print("PHASE2 DONE: ok=%d fail=%d" % (ok, fail))

# ============================================================
# PROMPTS for the 3 WORKFLOW apps
# ============================================================
PROMPTS_WF = {}

PROMPTS_WF["工业设备故障诊断工作流"] = """你是一名工业设备故障诊断专家,接收设备故障描述与运行参数,输出结构化的诊断报告。你精通旋转机械(电机/泵/风机/齿轮箱)、液压气动、电气驱动、过程设备的故障机理与诊断方法,熟悉ISO 10816/20816振动评估、FMEA、故障树(FTA)分析。

【诊断流程——严格按以下步骤推理】
1. 症状解析:从输入中提取故障现象(异响/振动/温升/压力异常/电流异常/停机)、发生工况(启停/稳态/变载)、设备型号与关键参数。
2. 故障假设生成:基于故障机理库列出3-5个可能原因,按概率排序(如:轴承磨损>不对中>不平衡>松动>电气故障),每个标注依据(特征频率、症状匹配度)。
3. 验证路径:为每个假设给出验证方法(振动频谱特征频率BPFO/BPFI、包络谱、油液分析、电流签名、温升速率),并说明判据。
4. 根因定位:综合证据给出最可能根因,标注置信度(高/中/低)与剩余不确定性。
5. 处置建议:给出处置措施(立即停机/降负荷运行/计划检修)、备件清单、维修工艺(拆装/对中/动平衡规范)、安全注意事项。
6. 预防复发:给出监测指标、阈值与巡检/定期维护建议,关联ISO 17359持续监测要求。

【输出格式——严格遵循】
请在回答中包含以下结构化段落:
【故障摘要】一句话概述设备+故障现象+初步判断。
【故障现象与工况】提取的客观信息。
【可能原因排序】编号列表,每项含依据与概率。
【验证方法】针对排序前列假设的检测手段与判据。
【根因结论】最可能根因+置信度。
【处置建议】措施+备件+维修要点+安全提示。
【预防措施】监测指标与维护计划。

对信息不足的情况,明确列出"需补充信息"清单,不臆测。"""
...
# Workflow apps: id -> (name, model, input_vars)
WF_APPS = [
    ("cbd7fa71-d8d1-45b5-bdf7-2c56d575669e", "工业设备故障诊断工作流", M_REASON,
     [{"variable":"fault_description","label":"故障描述与设备信息","type":"paragraph","required":True,"max_length":2000}]),
...
print()
print("=" * 60)
print("PHASE 3: Upgrading 3 WORKFLOW apps (workflow graph + prompt)")
print("=" * 60)
ok = 0; fail = 0
for aid, name, model, ivars in WF_APPS:
    prompt = PROMPTS_WF.get(name, "")
    if not prompt:
        print("  [SKIP] no prompt for %s" % name); fail += 1; continue
    graph = make_workflow_graph(prompt, model, ivars)
    features = {}  # workflow apps features handled differently (no chat features)
    code, resp = update_workflow_draft(aid, graph, features)
    plen = len(prompt)
    if code == 200:
        print("  [OK] %-26s model=%s prompt_len=%d" % (name, model, plen)); ok += 1
        ...
        print("  [FAIL] %-26s code=%d resp=%s" % (name, code, resp)); fail += 1
    time.sleep(0.5)
print("PHASE3 DONE: ok=%d fail=%d" % (ok, fail))
