...
> **历史快照（2026-09-07 测试时点）**：下表为测试当时的账户结构。现行体系（v3 矩阵）已调整：teacher-zhang 为**系统级教师测试账户**（非"总管理员"）；早期 teacher-b1-01 ~ teacher-p6-01 账户与 class-{课程}-{序号}-{班级} 分组已清理；A 课程主讲改为 teacher_ai_01（李智敏·班级1）/ teacher_ai_02（周成峰·班级2）；学生 2431 个按 `stu_{p,b,a}N_` 前缀自动挂载。详见 `TEACHING-MATRIX.md` 与 `ACCOUNT-SYSTEM-DESIGN-V2.md` §4.3。
