...
echo "=== 1. add postgres dep ==="
grep -q '^postgres' Cargo.toml || sed -i 's|^rusqlite = .*|&\npostgres = "0.19"|' Cargo.toml
grep -n "postgres\|rusqlite" Cargo.toml
echo "=== 2. write CrdbAuditRepository ==="
cat > src/infrastructure/crdb_repository.rs <<'RUST'
//! CockroachDB-backed repository (cluster deployments).
//! Same `AuditRepository` port; schema mirrors the SQLite tables.
use std::sync::Mutex;
...
impl AuditRepository for CrdbAuditRepository {
    fn save_session(&self, s: &AuditSession) -> Result<(), DomainError> {
        ...
        let tx = conn.transaction().map_err(|e| DomainError::Validation(format!("tx: {e}")))?;
        tx.execute(
...
RUST
echo "crdb_repository.rs: $(wc -l < src/infrastructure/crdb_repository.rs) lines"
echo "=== 3. register module ==="
grep -q crdb_repository src/infrastructure/mod.rs || echo 'pub mod crdb_repository;' >> src/infrastructure/mod.rs
cat src/infrastructure/mod.rs
echo "=== 4. switch server to CRDB ==="
...
use security_audit::infrastructure::crdb_repository::CrdbAuditRepository;

#[actix_web::main]
async fn main() -> std::io::Result<()> {
    env_logger::init_from_env(env_logger::Env::default().default_filter_or("info"));
    let repo = Box::new(CrdbAuditRepository::open()
        .expect("failed to connect to CRDB security_audit database"));
    log::info!("starting security-audit server on :8080 (CRDB backend, cluster mode)");
    run_server(repo, "0.0.0.0:8080").await
}
RUST
echo "server.rs switched to CRDB"
touch /tmp/rust_integrate_done
