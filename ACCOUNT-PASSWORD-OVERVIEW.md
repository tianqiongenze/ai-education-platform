...
| **A 课程主讲（新设）** | teacher-ai-01@edu.local / teacher-ai-02@edu.local | 经 `TEACHER_PASS` 环境变量注入，不落文件/DB | A 课程总主讲（李智敏/周成峰），A1~A12 全部 staff + JupyterHub 管理员，登录 Hub 自动获得 A 全套 12 份工单学生版+教师版及 12 个代码框架 starter |
| **Lecture 关联教师（2026-09-17 新设 64 个）** | teacher_a1_01/02 … teacher_a12_01/02、teacher_b1_01/02 … teacher_b12_01/02、teacher_p1_01/02 … teacher_p8_01/02（email：teacher-<lec>-0N@edu.local） | 经 `TEACHER_PASS` 环境变量注入，不落文件/DB | 每个 Lecture 2 名教师：_01 → <lec>-class1、_02 → <lec>-class2（staff + instructor 双角色），支持多教师并行/串行上同一 Lecture |

> 说明：teacher_zhang 保留为系统级教师测试账户。所有教师/学生密码仅通过 `TEACHER_PASS` / `STUDENT_PASS` / `ADMIN_PASS` 环境变量注入（冒烟脚本、测试套件运行时读取），不硬编码进任何文档或数据库。

#### 1.2.1a 师生账户口令规律（2026-09-18 全量重置后生效）

自 2026-09-18 起全部教师（64 个 Lecture 关联教师 + teacher_ai_01/02）与全部已挂载 Lecture 的学生/py 账户密码统一重置为**由账户名自派生**的规律口令，教师无需再向管理员索取。本文档仅记录规律，**不记录任何明文口令清单**：

| 账户类型 | 用户名示例 | 口令规律 | 示例 |
|----------|-----------|----------|------|
| Lecture 关联教师 | `teacher_a1_01` | `T` + `<lec>` + `@26`（`<lec>` 为小写课程号） | `Ta1@26` |
| A 课程总主讲 | `teacher_ai_01/02` | `Tai@26`（同教师规律，`<lec>`=ai） | `Tai@26` |
| 学生（stu_<lec>_*） | `stu_a1_002` | `<lec>` + `@26` | `a1@26` |
| py 账户（py_<lec>_*） | `py_a1_001` | `p` + `<lec>` + `@26` | `pa1@26` |

- 适用范围：`<lec>` ∈ a1…a12 / b1…b12 / p1…p8（32 个 Lecture），共 73 教师 + 1630 学生（含 py 账户）。
- **豁免（不在本规律内）**：系统账户 `teacher_zhang`、`admin`、`lecture-*` 系列仍保留原系统级口令（见 1.1 / 1.2.1 / 1.2.2 表）。
- 教师如忘记口令，可按上表自行推导；或由管理员用环境变量注入的脚本重置。
- 全量验证：220/220 账户登录扫描全部通过（2026-09-18，详见 `PLATFORM-GUIDE.md` §五b 与 `ACCOUNT-SYSTEM-DESIGN-V2.md` §9.8）。
