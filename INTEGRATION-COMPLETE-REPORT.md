...
> **状态**: ✅ 全部完成
>
> **现行差异说明（2026-09-16）**：本报告为 2026-09-09 时点快照。此后账户体系已升级（v3 矩阵）：teacher-zhang 调整为**系统级教师测试账户**（保留全 16 门课 staff）；A 课程主讲为 teacher_ai_01（李智敏·班级1）/ teacher_ai_02（周成峰·班级2）；A/B/P 课程内容按 32 份工单（A1~A12 / B1~B12 / P1~P8）重新同步。现行权威矩阵见 `TEACHING-MATRIX.md` 与 `ACCOUNT-SYSTEM-DESIGN-V2.md` §4.3。
...
## 七、LMS 课程面板修复 (2026-09-09 补充)

### 问题
用户登录 LMS 后课程面板显示"您尚未参加任何课程"，看起来"啥也没有"。

### 原因
admin 用户创建了16门课程并授权了教师角色，但**没有给自己选课**。LMS 面板只显示已选课的课程。

### 修复
为 admin 用户选课全部16门课程。选课后 LMS 面板正确显示全部课程。

### LMS 与 CMS 区别

| 维度 | LMS (openedx.*) | CMS/Studio (studio.*) |
|------|-----------------|----------------------|
| 角色 | 学生 | 教师 |
| 首页 | 课程面板 (已选课程) | 课程列表 (全部课程) |
| 功能 | 选课/学习/提交/成绩 | 创建/编辑/设置/管理 |
| 内容 | 只读消费 | 读写创建 |

### LMS 登录后功能
1. 课程面板 — 查看已选课程
2. 课程探索 — 浏览全部可选课程
3. 课程学习 — 查看讲义/实验内容 (含 JupyterHub/PrairieLearn 入口链接)
4. 课程进度 — 查看成绩 (含 PrairieLearn 回写分数)
5. OAuth 跳转 — 自动登录 JupyterHub
